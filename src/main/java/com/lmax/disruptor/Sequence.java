...
/**
 * 这里采用了填充来解决伪共享的问题,但是首先要看的并不是这个，而是当前类的作用
 * - long value:可以看到,当前类就是用来存储和管理序列号值的
 * 使用场景：
 *  - 生产者游标(Cursor)：记录生产者已发布的最高序号,消费者通过读取Cursor来判断是否有新事件
 *  - 消费者游标(Cursor)：记录消费者已处理的最高序号，生产者可以读取消费者序列来判断是否可以覆盖旧数据
 *  - Gating Sequences: 追踪所有消费者的处理进度，找到最慢的消费者，防止生产者覆盖掉未消费的数据
 */
